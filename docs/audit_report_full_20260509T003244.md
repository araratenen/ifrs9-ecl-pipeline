...
| Audit timestamp | `2026-05-09T00:32:44` |
...
|---|:---:|---|---|---|---|
| 1.1 | ✓ PASS | All expected files exist | all 29 files present | all present |  |
| 1.2 | ✓ PASS | All files loadable in expected format | all loadable | all loadable |  |
| 1.3 | ✓ PASS | No orphan/unexpected files | none | none |  |
...
| ID | Status | Description | Observed | Expected | Notes |
|---|:---:|---|---|---|---|
| 7.1 | ✗ FAIL | Binning fitted on training data only (count match) | binning saw -1, train.parquet=826604 | equal |  |
...
| 10.2 | ⚠ WARN | Random seeds documented where applicable | ['step9b_pd_model.py: random_state=42 not found'] | seeds set |  |
...
| ID | Status | Description | Observed | Expected | Notes |
|---|:---:|---|---|---|---|
| 12.1 | ✗ FAIL | All 16 methodological choices documented | missing=['4 macros + transformations'] | all documented |  |
...
| ID | Status | Description | Observed | Expected | Notes |
|---|:---:|---|---|---|---|
| 13.1 | ✓ PASS | test_predictions has columns Step 9c needs | ['default_flag', 'id', 'issue_d', 'pd_lr', 'pd_xgb'] | ['default_flag', 'id', 'issue_d', 'pd_lr', 'pd_xgb'] |  |
...
Fix the following before proceeding to Step 9c:

- **7.1 Binning fitted on training data only (count match)** — see category table above for observed vs expected.
- **12.1 All 16 methodological choices documented** — see category table above for observed vs expected.
...
If all 2 FAIL items are resolved, the pipeline is ready for Step 9c.
